...
export default function CertificateDownloader({
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      ...
      const listeningScore = result.score?.listening?.scaled ?? 31;
      const structureScore = result.score?.structure?.scaled ?? 31;
      const readingScore = result.score?.reading?.scaled ?? 31;
      ...
      doc.text(`Verify Online: kindenglish.id/verify/${result.id.slice(0, 8)}`, boxX + boxW - 6, boxY + 42.5, { align: "right" });
